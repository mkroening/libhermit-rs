...
use crate::arch::riscv64::kernel::mmio::*;
use crate::arch::riscv64::mm::{paging, PhysAddr, VirtAddr};
#[cfg(feature = "gem-net")]
		...
		unsafe {
			...
			if let Some(gem_node) = dtb.find_compatible(&["sifive,fu540-c000-gem"]) {
				...
				match gem::init_device(
					...
					Ok(drv) => register_driver(MmioDriver::GEMNet(InterruptSpinMutex::new(drv))),
					...
					if id != DevId::VIRTIO_DEV_ID_NET {
						...
						match mmio_virtio::init_device(mmio, irq.try_into().unwrap()) {
							Ok(VirtioDriver::Network(drv)) => {
								register_driver(MmioDriver::VirtioNet(InterruptSpinMutex::new(drv)))
							}
							Err(_) => (),
						}
					}
